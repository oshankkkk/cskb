...
**Core problem:** cloud apps (Google Docs, Trello, Figma) give you seamless collaboration but you don't actually own your data — the server is authoritative, you're a client with a cache. Old-fashioned apps (text editors, local files) give you full ownership but no real-time collaboration. The paper asks: can we get both?
...
1. **No spinners** — instant response since you're reading/writing local disk, not round-tripping to a server
...
## The proposed solution: CRDTs
**Conflict-free Replicated Data Types** — data structures (maps, lists, text) that are multi-user from the ground up and merge automatically, no central server needed to resolve conflicts. Each device holds a full local copy; changes sync via any channel (server, P2P, Bluetooth, USB) and merge deterministically. Similar spirit to Git, but operating on structured data instead of line-based text, and merging automatically instead of requiring manual resolution in the common case.

## Ink & Switch's findings from building 3 CRDT prototypes (Trellis/Kanban, PixelPusher/drawing, PushPin/canvas — all on their `automerge` library)
- CRDTs actually work reliably in practice — surprised them
- Offline UX was genuinely great — no anxiety about network state
- Conflicts were rarer than expected (fine-grained merge + humans naturally avoid stepping on each other)
- **Real problems found:** CRDTs accumulate unbounded history (perf/storage issue), network transport is a separate unsolved problem from the merge algorithm itself, P2P (NAT traversal etc.) is still rough, and visualizing "what changed and why" in a decentralized system is genuinely hard — there's no single source of truth to diff against

## Takeaways / calls to action
- Researchers: need branching/forking semantics for CRDTs (like git branches), and schema evolution without a central authority
- Practitioners: incrementally improve existing apps toward these ideals (aggressive caching, exportable formats, offline support)
- Their pitch to entrepreneurs: **"Firebase for CRDTs"** — a dev platform offering local persistence + sync + offline + multi-device, but where the server is never the source of truth. (Automerge itself, and later projects like Yjs and Kleppmann's own work, became attempts at this.)

---

Worth flagging: this is basically required reading for [[puffnotes]] — if you ever want the "not-a-generic-integration" creative feature, CRDT-based local-first sync (offline editing that merges cleanly across devices) is exactly the kind of architectural swing the paper is describing, and it'd give you real distributed-systems learning rather than just CRUD-on-AWS.