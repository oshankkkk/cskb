...
tags: []
---
"One after another" in your code is true — but it's *one after another in your program's execution*, which can be microseconds apart. That's not the same as "one after another on the wire," which is what actually determines separate `data()` calls.
